...
public class ItemGenerator2 : MonoBehaviour
{
    ...
    void Update()
    {
        var unityPos = (int)unityChan.transform.position.z;
        ItemGenerate(unityPos + 50);    //50m先に作る
    }

    void ItemGenerate(int itemPos)
    {
        if(itemPos < startPos) { return; }//startPosの前には作らない
        if(itemPos >= goalPos) { return; }//goalPos以降に作らない
        if(itemPos < lastPos + 15) { return; }//前回作成から15mは作らない
